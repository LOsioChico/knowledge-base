...
source:
  ...
  - https://github.com/Effect-TS/effect/blob/main/packages/sql/README.md
...
`@effect/sql`, `@effect/ai`, and `@effect/platform` are the lines that fan out: SQL and AI into per-driver / per-provider sub-packages (tables below), and platform into runtime adapters (`@effect/platform-node`, `-bun`, `-browser`; see the adapter table). Most other `@effect/*` packages are one npm install each.
...
| `@effect/sql`      | "A SQL toolkit for Effect" ([package.json](https://github.com/Effect-TS/effect/blob/main/packages/sql/package.json)): tagged-template queries via `` sql`...` `` ([README example](https://github.com/Effect-TS/effect/blob/main/packages/sql/README.md)), `Layer`-managed clients, schema-typed rows.                                                         | You want type-safe SQL with `Layer`-managed connections and rollbacks tied to fiber lifetime.                                      | You're already heavily invested in TypeORM/Prisma and not ready to migrate; see the per-driver adapters below before deciding.              |
...
| `@effect/workflow` | "Durable workflows for Effect" ([README](https://github.com/Effect-TS/effect/blob/main/packages/workflow/README.md)): replayable, resumable workflows; the README wires `ClusterWorkflowEngine` from `@effect/cluster` with `PgClient` from `@effect/sql-pg` ([example](https://github.com/Effect-TS/effect/blob/main/packages/workflow/README.md#L121-L130)). | Long-running multi-step processes that must survive restarts (order processing, billing pipelines, signup flows with email steps). | Short-lived in-memory work; sub-second tasks; anything where a Temporal-style engine is overkill.                                           |
