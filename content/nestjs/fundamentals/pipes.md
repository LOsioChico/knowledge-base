...
tags: [type/concept, lifecycle, validation, gotchas]
area: nestjs
status: evergreen
related:
...
| `ParseIntPipe`     | string → integer                                             | Regex `^-?\d+$` ([`isNumeric`](https://github.com/nestjs/nest/blob/master/packages/common/pipes/parse-int.pipe.ts#L80-L86)). Throws `BadRequestException` by default. See [composing pipes](#common-recipes)                                                                                                                      |
...
The global-scope variant of the same DI question: `useGlobalPipes(new X())` vs `APP_PIPE`: has its own dedicated note: [[nestjs/fundamentals/global-providers|Global pipes, guards, interceptors, and filters via DI]]. It covers the side-by-side comparison, request-scope (one provider instance per inbound HTTP request) and hybrid-app (one Nest process serving multiple transports, e.g. HTTP plus a microservice listener) implications, and when to reach for `useClass` vs `useFactory`.
...
| `forbidUnknownValues`   | `false`     | Reject unknown objects. Nest forces `false` ([`validation.pipe.ts#L76`](https://github.com/nestjs/nest/blob/master/packages/common/pipes/validation.pipe.ts#L70-L80)) even though `class-validator`'s own default is `true` ([issue #10683](https://github.com/nestjs/nest/issues/10683)) |
