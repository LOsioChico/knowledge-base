...
| ------------------------------------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| [Helmet](https://docs.nestjs.com/security/helmet)             | `helmet`          | Security headers (CSP: Content Security Policy, HSTS: HTTP Strict Transport Security, X-Frame-Options, …) | `app.use(helmet())` in `main.ts` ([example](#common-recipes))                                 |
...
> Express v5 is the default in NestJS 11 ([migration guide → Express v5](https://github.com/nestjs/docs.nestjs.com/blob/master/content/migration.md#express-v5)) and requires **named** wildcards. The same path-to-regexp v8 matcher applies to the Fastify adapter, which NestJS 11 also bumped ([Fastify middleware registration](https://github.com/nestjs/docs.nestjs.com/blob/master/content/migration.md#L114-L131)). Two shapes you'll actually use, identical semantics on both adapters:
>
> - `'*splat'` matches any path **below** the base, root excluded ([`migration.md#L41`](https://github.com/nestjs/docs.nestjs.com/blob/master/content/migration.md#L41)). The docs' Fastify example uses this form when migrating from `'(.*)'`.
> - `'{*splat}'` matches any path **including** the bare base; outer braces make the wildcard optional ([`migration.md#L57`](https://github.com/nestjs/docs.nestjs.com/blob/master/content/migration.md#L57)). Use this when "all routes" must include `/`.
>
> `splat` is just a parameter name; pick any identifier. Pre-v11 patterns like `'*'`, `'(.*)'`, and `'users/*'` still work via Nest's compatibility layer but emit warnings; the migration is mechanical. Same fix applies to `@Get('*')` and `RouterModule` paths.
