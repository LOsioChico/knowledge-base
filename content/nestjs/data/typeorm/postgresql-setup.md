...
| `ssl`                 | TLS to the server                     | `false`                             | Set to `{ rejectUnauthorized: false }` for managed Postgres providers with self-signed certs (Supabase, Neon, AWS-hosted, etc.); tighten for prod |
