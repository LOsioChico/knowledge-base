...
- [Telerik: Learning NestJS Part 3, Data Caching](https://www.telerik.com/blogs/learning-nestjs-part-3-data-caching): extended walkthrough that includes an Azure Cache for Redis setup. Note: the post is internally inconsistent on TTL units. Its `CacheModule.register({ ttl: 30000 })` correctly uses milliseconds (30s), but the route-override example `@CacheTTL(60)` reads like the pre-v3 seconds-based API. On `@nestjs/cache-manager@3.x` (the current major) `@CacheTTL` takes **milliseconds**, so copy that line as-is and you get a 60 ms cache (effectively none); use `@CacheTTL(60_000)` for the intended 60 seconds.
