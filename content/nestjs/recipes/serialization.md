...
> [!warning]- Plain objects bypass the decorator strip (unless you opt in)
> The most common bug, recapped here because it's how every leak in this recipe happens. If the controller returns a plain object literal (or anything not `instanceof YourEntity`), `ClassSerializerInterceptor` still calls `class-transformer`'s `classToPlain`, but `class-transformer` keys decorator behavior off the **class metadata** of the value: a plain object has no class, so `@Exclude()` matches nothing and every field reaches the wire (per the [Nest docs](https://docs.nestjs.com/techniques/serialization#overview): "we must return an instance of the class. If you return a plain JavaScript object... the object won't be properly serialized"). Either `return new Entity(...)` / `plainToInstance(Entity, raw)`, or annotate the route with `@SerializeOptions({ type: Entity })` so the interceptor converts the plain object before serializing. See [the class-instance gotcha](#the-class-instance-gotcha) for the ORM-specific cases.
