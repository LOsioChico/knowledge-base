...
| `blockDuration`    | `number` (ms)                             | `ttl`       | How long to keep blocking after the limit is hit. Falls back to `ttl` when unset ([`throttler.guard.ts#L122-L124`](https://github.com/nestjs/throttler/blob/master/src/throttler.guard.ts#L122-L124): `routeOrClassBlockDuration \|\| namedThrottler.blockDuration \|\| ttl`) |
...
> The default response body is `{ "statusCode": 429, "message": "ThrottlerException: Too Many Requests" }`. The guard sets headers automatically when `setHeaders` is on (defaults to `true`): on **allowed** responses, `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`. On a **blocked** `429`, only `Retry-After` (value comes from `timeToBlockExpire`): `handleRequest` sets that header inside the `isBlocked` branch and then throws via `throwThrottlingException`, never reaching the counter-headers block ([`throttler.guard.ts#L179-L208`](https://github.com/nestjs/throttler/blob/master/src/throttler.guard.ts#L179-L208)). To customize the body, override via the `errorMessage` option or write a small filter on `ThrottlerException`.
