...
For cross-account `s3 sync`, the source bucket policy must grant the target account `s3:GetObject` and `s3:ListBucket`; full walkthrough in [[aws/recipes/cross-account-bucket-migration|cross-account bucket migration]].
