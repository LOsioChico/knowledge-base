...
> [[aws/cloudfront/index|CloudFront]] alternate domain names are globally unique across accounts; the documented way to take an alias from another distribution is `update-domain-association`, but if you can't reach that path ([[aws/amplify/index|Amplify]]-managed flow, source unreachable) presenting your own ACM certificate is the empirical workaround.
