...
The hosting layer underneath is CloudFront: every Amplify app gets a CloudFront distribution that you don't see in `cloudfront list-distributions` from your account. You manage it through `aws amplify` instead. See [[aws/cloudfront/index|CloudFront]] for what's actually serving the bytes.
