...
4. **Management and analytics.** S3 Storage Lens (org-wide storage metrics dashboard), Inventory (scheduled CSV/Parquet listing of objects), Batch Operations (managed bulk-action jobs), and Object Tagging (per-object key-value labels used for cost allocation, lifecycle filters, and IAM conditions) each have their own per-object or per-feature fee. Off by default; only billed when enabled.
