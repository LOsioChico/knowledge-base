...
> **Step 9: Concurrency & Fibers**: Structured concurrency in Effect. Asynchronous units of execution called **fibers** are organized in a strict parent-child hierarchy, guaranteeing no leaked operations or abandoned computations. Prerequisite: [[effect-ts/scoped-resources|Step 8: Scoped resources]]; next step: [[effect-ts/streams|Step 10: Streams]].
...
```ts twoslash title="fork-background.ts"
...
<FiberTimeline
  title="Fork → Work → Join lifecycle"
  lanes={[
    { id: "parent", label: "Parent", color: "#60a5fa" },
    { id: "child", label: "Child", color: "#a78bfa" },
  ]}
  segments={[
    // t0 : parent starts
    { lane: "parent", start: 0, end: 0, state: "active", label: "start", tip: "Parent fiber begins executing the Effect.gen block." },
    // t1 : fork: parent continues, child starts
    { lane: "parent", start: 1, end: 1, state: "active", label: "fork + log", tip: "Effect.fork(backgroundJob) spawns the child fiber. Parent immediately continues with Console.log : no waiting." },
    { lane: "child",  start: 1, end: 3, state: "active", label: "background job", tip: "Child runs in parallel: sleep(100ms), Console.log, then returns 42." },
    // t2–t3 : parent joins, waits for child
    { lane: "parent", start: 2, end: 3, state: "blocked", label: "join : waiting", tip: "Fiber.join(fiber) blocks the parent until the child completes. This is where concurrency meets synchronization." },
    // t4 : child done, parent resumes
    { lane: "parent", start: 4, end: 4, state: "active", label: "got 42", tip: "Child returned 42 → parent unblocks, receives the value, and logs it." },
  ]}
  events={[
    { time: 1, from: "parent", to: "child", label: "fork" },
    { time: 3, from: "child", to: "parent", label: "42" },
  ]}
  steps={5}
/>
...
```ts twoslash title="fiber-lifecycle.ts"
import { Console, Effect, Fiber } from "effect";
...
| --- | --- | --- |
| `undefined` | Default | **Sequential**: Runs each effect in order, waiting for the previous to finish (see [Sequential example](#sequential-vs-parallel-comparison)). |
| `"unbounded"` | string | **Unbounded Parallel**: Spawns all effects concurrently as child fibers immediately (see [Parallel example](#sequential-vs-parallel-comparison)). |
| `N` | number | **Bounded Parallel**: Spawns at most `N` concurrent child fibers, queueing the rest (see [Bounded limits example](#bounded-concurrency-limits)). |
| `"inherit"` | string | **Inherited Parallel**: Inherits the concurrency configuration of enclosing context (see [Inherited scope example](#contextual-inherited-concurrency)). |

### Sequential vs. Parallel comparison

<Tabs>
  <TabItem label="Sequential (Default)">
    ```ts twoslash title="sequential-vs-parallel.ts"
  ...
  <TabItem label="Parallel (Unbounded)">
    ```ts twoslash title="parallel-variants.ts"
    import { Console, Effect } from "effect";
...
```ts twoslash title="bounded-concurrency.ts"
...
```ts twoslash title="inherited-concurrency.ts"
import { Console, Effect } from "effect";
...
```ts twoslash title="on-interrupt.ts"
import { Console, Effect, Fiber } from "effect";
...
```ts twoslash title="guarded-scope.ts"
import { Console, Effect, Fiber } from "effect";
