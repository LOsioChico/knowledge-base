...
```mermaid
flowchart TD
    QS["1. [[effect-ts/quickstart|Quickstart]] (Entry)"] --> WE["2. [[effect-ts/what-is-effect|What is Effect]]"]
    WE --> COMP["3. [[effect-ts/composition|Composition]]"]
    COMP --> ERR["4. [[effect-ts/typed-errors|Typed Errors]]"]
    ERR --> SCH["5. [[effect-ts/schema|Schema]]"]
    SCH --> LYR["6. [[effect-ts/layers-and-di|Layers & DI]]"]
    LYR --> RET["7. [[effect-ts/retry-and-schedule|Retry & Schedule]]"]
    RET --> SCP["8. [[effect-ts/scoped-resources|Scoped Resources]]"]
    SCP --> CONC["9. [[effect-ts/concurrency|Concurrency]]"]
    CONC --> STR["10. [[effect-ts/streams|Streams]]"]
    STR --> ST["11. [[effect-ts/state|State Coordinates]]"]
    ST --> OBS["12. [[effect-ts/observability|Observability]]"]
    OBS --> PLT["13. [[effect-ts/platform|Platform Layers]]"]
    PLT --> FT["14. [[effect-ts/fault-tolerant-ingestion|Ingestion Capstone]]"]
```
