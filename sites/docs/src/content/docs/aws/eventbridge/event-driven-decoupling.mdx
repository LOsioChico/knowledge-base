...
| **Payload Filtering** | **None** (Consumers must parse the entire body or filter using raw metadata attributes). | **Metadata attributes only** (Filter policies matching numeric or string arrays). | **Deep JSON matching** (Filter patterns matching deep nested objects in event bodies). |
