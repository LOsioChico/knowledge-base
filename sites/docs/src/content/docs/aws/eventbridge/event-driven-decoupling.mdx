...
In distributed architectures, contract drift is a major footgun. If a publisher alters its event structure, downstream consumers lacking a [[nestjs/recipes/validation|validation]] boundary may operate on unexpected null or undefined values or throw unhandled runtime exceptions. 
