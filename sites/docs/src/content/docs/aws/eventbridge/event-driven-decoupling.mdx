...
This note is part of the **[[aws|AWS Serverless Track]]**. Before designing decoupled event-driven systems, make sure you understand:
- [[aws/eventbridge|EventBridge Concept Index]]: Serverless event bus fundamentals and basic mechanics.
...
- [[aws/iam|IAM Policies & Principal Taxonomy]]: Multi-account trust relationships, resource-based policies, and cross-account IAM delegation.
- [[effect-ts/schema|Effect Schema validation]]: Essential type-safe validation constructs (like TaggedError and decodeUnknown) used in downstream schemas.
...
- **Event Bus (Conveyor Belt)**: A central stream where all incoming events circulate.
...
graph TD
    ...
    click Bus "/knowledge-base/aws/eventbridge/"
    click SQSBilling "/knowledge-base/aws/sqs/"
    click SQSInv "/knowledge-base/aws/sqs/"
    click SNSNotify "/knowledge-base/aws/sns/"
```

<FlowSimulator
  title="Event routing animation"
  nodes={[
    { id: "order", label: "Order Service", col: 0 },
    { id: "bus", label: "Event Bus", col: 1 },
    { id: "rule-billing", label: "Billing Rule", col: 2 },
    { id: "rule-inv", label: "Inventory Rule", col: 2 },
    { id: "sqs-billing", label: "Billing Queue", col: 3 },
    { id: "sqs-inv", label: "Inventory Queue", col: 3 },
    { id: "billing", label: "Billing Service", col: 4 },
    { id: "inventory", label: "Inventory Service", col: 4 },
  ]}
  flows={[
    {
      label: "ORDER_COMPLETED (happy path)",
      steps: [
        { from: "order", to: "bus", message: "PutEvents(ORDER_COMPLETED)" },
        { from: "bus", to: "rule-billing", message: "Pattern match ✓", group: 1 },
        { from: "bus", to: "rule-inv", message: "Pattern match ✓", group: 1 },
        { from: "rule-billing", to: "sqs-billing", message: "Push to queue", group: 2 },
        { from: "rule-inv", to: "sqs-inv", message: "Push to queue", group: 2 },
        { from: "sqs-billing", to: "billing", message: "Pull & process", group: 3 },
        { from: "sqs-inv", to: "inventory", message: "Pull & process", group: 3 },
      ],
    },
    {
      label: "Unmatched event (filtered)",
      steps: [
        { from: "order", to: "bus", message: "PutEvents(ORDER_FAILED)" },
        { from: "bus", to: "rule-billing", message: "Pattern match ✗", group: 1 },
        { from: "bus", to: "rule-inv", message: "Pattern match ✗", group: 1 },
      ],
    },
...
In distributed architectures, contract drift is a major footgun. If a publisher alters its event structure, downstream consumers lacking a validation boundary may operate on unexpected null or undefined values or throw unhandled runtime exceptions. 
...
### Failure Mode (Show-Don't-Tell)

When a publisher violates the contract by omitting the mandatory `amount` field, downstream services receive a malformed payload:

<Tabs>
  <TabItem label="Malformed Inbound Event">
    ```json
    {
      "id": "evt_987654321",
      "source": "mycompany.sales",
      "detail-type": "OrderCompleted",
      "time": "2026-05-23T18:27:00Z",
      "region": "us-east-1",
      "detail": {
        "orderId": 5012,
        "customerEmail": "vip@gmail.com"
      }
    }
    ```
  </TabItem>
  <TabItem label="Validation Error Output">
    ```text
    EventValidationError: EventBridgeEnvelope
...
<Aside type="caution" title="Validate at the ingress boundary only">
Re-validating event payloads at multiple internal hops wastes CPU cycles and hazards logic drift where different service tiers enforce conflicting schemas. Execute the validation exactly once at your consumer's entry-point controller or SQS queue listener, then propagate the clean, validated `EventBridgeEnvelope` type throughout your internal application layers.
</Aside>
...
<FlowSimulator
  title="Secure cross-account event routing"
  nodes={[
    { id: "AccountABus", label: "Event Bus (Account A)", col: 0 },
    { id: "AccountARule", label: "Routing Rule (Account A)", col: 0 },
    { id: "AccountBBus", label: "Target Event Bus (Account B)", col: 1 },
  ]}
  flows={[
    {
      label: "Cross-account routing",
      steps: [
        { from: "AccountABus", to: "AccountARule", message: "Evaluates Event Pattern", description: "Rule filters match the inbound event, matching the destination pattern." },
        { from: "AccountARule", to: "AccountBBus", message: "PutEvents with Assumed IAM Role", description: "Rule assumes a cross-account IAM role and publishes events directly to Account B's bus." },
      ],
    },
  ]}
/>
...
- [Amazon EventBridge Event Patterns](https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-event-patterns.html) (official): Pattern syntax reference for the [deep JSON matching](#deep-json-pattern-matching) section.
