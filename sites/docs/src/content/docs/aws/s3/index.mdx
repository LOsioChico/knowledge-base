...
> Amazon S3 is object storage: put bytes in a bucket under a string key; S3 makes them durable, regional, and addressable over [[effect-ts/platform|HTTP]]. Default place for anything that is not a database row.
...
- **Concurrent same-key PUTs: last write wins unpredictably**: not a [[effect-ts/concurrency|concurrency]] lock; don't use S3 Object Lock for that ([source](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lock.html)).
...
Block Public Access at account + bucket ([source](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-control-block-public-access.html)). If a request still gets `AccessDenied`, debug the identity policy, bucket policy, and guardrail layers with [[aws/iam/policy-evaluation|IAM policy evaluation]].
...
| [Storage classes](/knowledge-base/aws/s3/storage-classes/) | [Storage classes](/knowledge-base/aws/s3/storage-classes/)                 |
