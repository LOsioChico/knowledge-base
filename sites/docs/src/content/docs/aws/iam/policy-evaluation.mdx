...
| --------------------- | -------------------------------------------------------------------------------------- | ------------------------------ | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identity-based policy | Attached to a user, group, or role                                                     | Yes                            | Yes, with explicit `Deny` | The error says no identity-based policy allows the action ([source](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html#policies_id-based)).                           |
| Resource-based policy | Attached to a resource, such as an S3 bucket, SQS queue, KMS key, or role trust policy | Yes                            | Yes, with explicit `Deny` | The resource owner controls access, especially cross-account access ([source](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html#policies_resource-based)).           |
| Permissions boundary  | Managed policy attached as a maximum-permissions boundary on an IAM user or role       | No                             | Yes                       | Identity policies allow the action, but the principal still cannot perform it ([source](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_boundaries.html)).              |
...
| Trust policy          | Resource-based policy on an IAM role                                                   | Yes, for `sts:AssumeRole` only | Yes                       | `AssumeRole` fails before the target service call happens ([source](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html)).                                           |
...
Result: if either side is missing, `AssumeRole` fails before the app reaches S3, KMS, SES, or any other target service. The STS `AssumeRole` docs [[effect-ts/state|state]] that the target role trust policy defines who can assume the role, and cross-account assumption requires the caller's account to be trusted by that role ([source](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html)).
...
| Field                  | Meaning                                                                                                                                                          |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `EvalDecision`         | `allowed`, `explicitDeny`, or `implicitDeny`. Start here ([source](https://docs.aws.amazon.com/cli/latest/reference/iam/simulate-principal-policy.html#output)). |
| `MatchedStatements`    | Which statement decided the result. For explicit deny, the deny statement is the useful one.                                                                     |
| `MissingContextValues` | Condition keys the simulator needed but did not receive. Add `--context-entries` when the real request depends on keys such as `aws:SourceIp` or `s3:prefix`.    |
