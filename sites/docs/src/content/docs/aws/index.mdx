...
```mermaid
flowchart TD
    CLI["1. [[aws/cli/profiles-and-credentials|CLI Credentials]] (Entry)"] --> QRY["2. [[aws/cli/query-and-output|Query & Output]]"]
    QRY --> S3QS["3. [[aws/s3/quickstart|S3 Quickstart]]"]
    S3QS --> S3SC["4. [[aws/s3/storage-classes|Storage Classes]]"]
    S3SC --> S3LC["5. [[aws/s3/lifecycle-rules|S3 Lifecycles]]"]
    S3LC --> EB["6. [[aws/eventbridge/event-driven-decoupling|EventBridge Asynchronous Decoupling]]"]
```
