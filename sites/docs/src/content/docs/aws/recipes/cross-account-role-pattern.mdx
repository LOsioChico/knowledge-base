...
<FlowSimulator
  ...
  nodes={[
    { id: "b", label: "Account B (Compute)", col: 1 },
    { id: "sts", label: "STS (Account A)", col: 2 },
    { id: "role", label: "IAM Role (Account A)", col: 3 },
    { id: "svc", label: "Service (Account A)", col: 4 }
  ]}
  flows={[
    {
      label: "AssumeRole Sequence",
      steps: [
        { from: "b", to: "sts", message: "sts:AssumeRole", description: "Account B's compute requests temporary credentials by calling sts:AssumeRole, passing the target Role ARN and the required ExternalId." },
        { from: "sts", to: "b", message: "Temp Credentials", description: "STS validates the trust policy and ExternalId in Account A. Upon success, it returns short-lived, temporary AWS credentials." },
        { from: "b", to: "role", message: "API Call", description: "Account B invokes the target service using the temporary credentials, effectively assuming the identity of the Cross-Account Role in Account A." },
        { from: "role", to: "svc", message: "ses:SendEmail", description: "AWS validates that the role's permission policy allows the target action (e.g. sending emails via SES) and forwards the call." },
        { from: "svc", to: "b", message: "Response", description: "The target service processes the request and sends the response back to the client in Account B." }
      ]
    }
  ]}
/>
