...
<FlowSimulator
  title="Trace ID propagation through the request lifecycle"
  nodes={[
    { id: "client", label: "Client" },
    { id: "middleware", label: "TraceMiddleware" },
  ...
  flows={[
    {
      label: "Happy path",
      steps: [
        { from: "client", to: "middleware", message: "GET /cats : x-request-id: (none)" },
        { from: "middleware", to: "middleware", message: "traceId = randomUUID() → als.run({ traceId }, next)" },
        { from: "middleware", to: "guard", message: "ALS context open : traceId: 8f2a4c6e" },
        { from: "guard", to: "interceptor", message: "Guard passes : traceId readable via getTraceId()" },
        { from: "interceptor", to: "handler", message: "TimingInterceptor starts timer : traceId: 8f2a4c6e" },
        { from: "handler", to: "logger", message: "this.logger.log('list() called') → [8f2a4c6e] LOG" },
        { from: "handler", to: "client", message: "200 : x-request-id: 8f2a4c6e in response header" },
      ],
    },
    {
      label: "Error path",
      steps: [
        { from: "client", to: "middleware", message: "GET /cats/999 : x-request-id: custom-tx" },
        { from: "middleware", to: "middleware", message: "Inbound header found → traceId = 'custom-tx'" },
        { from: "middleware", to: "guard", message: "ALS context open : traceId: custom-tx" },
        { from: "guard", to: "handler", message: "Guard + interceptor pass through" },
        { from: "handler", to: "handler", message: "throw NotFoundException('Cat 999 not found')" },
        { from: "handler", to: "logger", message: "TraceExceptionFilter catches → logs { traceId: 'custom-tx', status: 404 }" },
        { from: "handler", to: "client", message: "404 : { traceId: 'custom-tx', message: 'Cat 999 not found' }" },
      ],
    },
    {
      label: "Outbound propagation",
      steps: [
        { from: "client", to: "middleware", message: "POST /orders : traceId: abc-123" },
        { from: "middleware", to: "handler", message: "ALS context → traceId: abc-123" },
        { from: "handler", to: "handler", message: "Handler calls external API via HttpService" },
        { from: "handler", to: "logger", message: "Axios interceptor adds x-request-id: abc-123 to outbound request" },
        { from: "logger", to: "handler", message: "External API response received : same trace ID propagated" },
        { from: "handler", to: "client", message: "201 : x-request-id: abc-123" },
      ],
    },
  ]}
/>
...
<Aside type="caution" title="TraceLogger should be Transient Scope">

`AsyncLocalStorage` itself works correctly regardless of instance scope: `getTraceId()` always resolves the store for the current async context. However, NestJS's `ConsoleLogger` uses the `context` string (e.g. `[CatsController]`) set at injection time. With `Scope.DEFAULT`, every consumer shares the same instance and the same context name. Use `Scope.TRANSIENT` so each injecting class gets its own logger instance with the correct context name.
