...
No external npm packages are required for the core correlation pattern. `AsyncLocalStorage` is part of the Node.js standard library. Optionally, you can use `@nestjs/axios` for outbound [[effect-ts/platform|HTTP]] propagation (step 5).
