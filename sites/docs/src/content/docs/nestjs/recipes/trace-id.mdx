...
Ecohing any arbitrary `X-Request-ID` from the client allows malicious users to pollute log indexing with massive payloads or collisions. If your application faces the open internet, strip client headers at your load balancer or gateway, or configure your middleware to accept headers only from trusted upstream CIDR blocks.
