...
| Form         | When                                                 |
| ------------ | ---------------------------------------------------- |
| `useClass`   | Nest resolves every constructor dep (worked example: [Request-scoped APP_GUARD](#request-scoped-app-guard)) |
| `useValue`   | Pre-built instance, e.g. `new ValidationPipe({ … })` (worked example: [combined array](#provider-shapes)) |
| `useFactory` | Async setup, env, or manual `inject: [...]` (worked example: [Config-driven APP_PIPE](#config-driven-app-pipe)) |
...
`app.useGlobalGuards()` / `useGlobalPipes()` / etc. apply to the **[[effect-ts/platform|HTTP]]** layer only. Microservice listeners and WebSocket gateways stay uncovered unless you pass `{ inheritAppConfig: true }` to `connectMicroservice`, or register via `APP_*` so Nest's enhancer registries pick up globals on every transport ([hybrid application FAQ](https://docs.nestjs.com/faq/hybrid-application)).
...
- **Multiple `APP_*` registrations stack**: e.g. [[effect-ts/observability|logging]] + timeout interceptors both run.
