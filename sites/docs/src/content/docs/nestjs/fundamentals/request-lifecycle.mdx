...
| **3** | **Guards** | Nest Enhancer | Token authentication (JWT verify), role-based authorization, rate limiting (throttling). | Yes (Nest Exception Filter) |
| **4** | **Interceptors (Pre)**| Nest Enhancer | Request caching checks, start timer (performance profiling), request logging. | Yes (Nest Exception Filter) |
| **5** | **Pipes** | Nest Enhancer | Input validation (`class-validator`), data parsing / coercion (`ParseIntPipe`). | Yes (Nest Exception Filter) |
| **6** | **Controller** | Business Logic | Executing target database queries, service delegation, mapping response objects. | Yes (Nest Exception Filter) |
| **7** | **Interceptors (Post)**| Nest Enhancer | Mapping / transforming response objects, profiling duration telemetry, logs formatting. | Yes (Nest Exception Filter) |
