...
| **4** | **Interceptors (Pre)**| Nest Enhancer | Request [caching](/knowledge-base/nestjs/data/caching/) checks, start timer (performance profiling), request logging (see [One route, all layers](#one-route-all-layers)). | Yes (Nest Exception Filter) |
