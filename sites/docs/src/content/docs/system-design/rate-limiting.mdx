...
<DecisionGuide
  ...
  steps={[
    ...
    { id: "approx", result: "Token Bucket", explanation: "Token Bucket allows full bursts up to capacity, then enforces a steady refill rate. Low memory, simple to implement, the standard for API gateways." },
    { id: "r-leaky", result: "Leaky Bucket", explanation: "Leaky Bucket enforces perfectly smooth output. Requests queue up and drain at a fixed rate. Best for worker queues and background job processing." },
    { id: "r-sliding-log", result: "Sliding Window Log", explanation: "Stores every request timestamp for exact counting. Perfect accuracy but high memory cost — O(n) per user. Good for low-volume high-value APIs." },
    { id: "r-sliding-counter", result: "Sliding Window Counter", explanation: "Approximates the sliding window using weighted fixed-window counts. Near-exact accuracy with O(1) memory. The production sweet spot for most gateways." },
  ]}
/>
...
<CodeWalkthrough
  ...
  steps={[
    ...
    { lines: "12-14", title: "Snapshot the window", body: "Each call captures the current time and computes a boundary. Any request timestamp older than `now - windowMs` is outside the active window." },
    ...
    { lines: "35-47", title: "Background pruning", body: "For long-running servers, call pruneExpired() on an interval (e.g., every 60 seconds). It walks every client's log, removes expired timestamps, and deletes map entries that are now empty." },
  ]}
>
