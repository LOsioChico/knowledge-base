...
This note is part of the **[Systems Design Track](/knowledge-base/system-design/)**. Before diving into rate limiting and traffic shaping, make sure you understand:
...
*   **Burst Support**: None. It smooths out bursty traffic into a completely flat, predictable [[effect-ts/streams|stream]] of processing.
...
| Algorithmic Criteria | Token Bucket | Leaky Bucket | Fixed Window | Sliding Log | Sliding Counter |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
| **Primary Technology Fit** | API Gateways | Worker Queues | Simple Scripts | Redis Sorted Sets | Production Gateways |

*Note: For detail on how standard clients use these concepts, check the [NestJS Rate Limiting Recipe](/knowledge-base/nestjs/recipes/rate-limiting/).*
...
*   *Cons*: Entirely untrusted. Any client can bypass the logic by calling the [[effect-ts/platform|HTTP]] endpoint directly or spoofing the client SDK.
...
Application servers invoke a shared, centralized cache like Redis to store and evaluate rate limiting [[effect-ts/state|state]].
