...
<CodeWalkthrough
  ...
  steps={[
    { lines: "1-2", title: "State storage", body: "A Map keyed by client ID stores an array of Unix-ms timestamps — one entry per allowed request. This is the 'log' in Sliding Window Log." },
    ...
    { lines: "16-18", title: "Fetch and evict stale entries", body: "Pull the client's existing timestamps from the Map, then filter out everything older than the boundary. The window 'slides' forward with every request." },
    ...
    { lines: "26-27", title: "Reject but still clean", body: "Over the limit — return false. The pruned array is still saved so stale entries don't accumulate on future checks." },
