...
- **Rate Limiting & Traffic Shaping**: Implementing token bucket, leaky bucket, and sliding window counter algorithms at the API Gateway layer to defend downstream services against DDoS attacks.
- **Distributed Caching & Invalidation**: Master-replica caching architectures, invalidation models (cache-aside, write-through, write-behind), eviction policies (LRU, LFU), and cache stampede mitigations.
