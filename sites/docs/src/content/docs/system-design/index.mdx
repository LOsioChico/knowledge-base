...
For a progressive learning flow, we recommend mastering quantitative estimation before diving into the CAP consistency spectrum (consistency models), event coordination (logical clocks), and data distribution rings (consistent hashing):

```mermaid
flowchart TD
    EST["1. [[system-design/back-of-the-envelope-estimation|Estimation Primitives]] (Entry)"] --> CM["2. [[system-design/consistency-models|Consistency Models]]"]
    CM --> LC["3. [[system-design/logical-clocks|Logical Clocks]]"]
    CM --> CH["4. [[system-design/consistent-hashing|Consistent Hashing]]"]
```
...
<Tabs>
  <TabItem label="Core Architecture">
    ...
    4. [[system-design/consistent-hashing|Consistent Hashing]]: Master data partitioning, circular hashing rings, and hotspot mitigation with virtual nodes.
  ...
  <TabItem label="Algorithmic Primitives">
    ...
    2. **TypeScript Implementation**: Walk through a fully typed hash ring class using MD5 and O(log N) binary search: [[system-design/consistent-hashing#typescript-implementation|Consistent Hashing Code]].
    3. **Causal Timeline Verifications**: Compare vector clocks and trace event ordering: [[system-design/logical-clocks#visualizing-logical-clocks|Causal Tracing]].
