...
This note is part of the **[Systems Design Track](/knowledge-base/system-design/)**. Before resolving multi-service [[effect-ts/state|state]] consistency and transaction choreography, make sure you understand:
- [[system-design/consistent-hashing|Consistent Hashing]]: Master dynamic partition distribution rings and virtual node data load balancing.
...
*   **The Mitigation**: Every command must carry a unique Saga ID and Step ID. Before processing, services must verify if the specific Step ID has already run, rejecting duplicate invocations. (See [Asynchronous Message Queues](/knowledge-base/system-design/message-queues/) for details on distributed idempotency locks).
...
*   [[nestjs/recipes/rate-limiting|NestJS Rate Limiting]]: Implement gateway rate limiting to prevent traffic floods from overwhelming multi-service transaction queues.
