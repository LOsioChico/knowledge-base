...
> Foundational concepts and architectural patterns for coordinating atomic multi-service updates, contrasting locking protocols with eventual consistency sagas, and securing reliable event routing.

import { Aside, Steps, Tabs, TabItem } from "@astrojs/starlight/components";
import FlowSimulator from '/src/components/interactive/FlowSimulator.astro';
import SteppedDiagram from '/src/components/interactive/SteppedDiagram.astro';
...
The Two-Phase Commit (2PC) protocol is a classic distributed consensus pattern that guarantees absolute atomicity across multiple nodes. It utilizes a central coordinator process to direct the transaction lifecycle across several participant databases.
...
A Saga coordinates a business transaction through a sequence of local transactions. Each step in the Saga executes a local transaction in a single service and commits instantly, releasing database locks immediately. 
...
<SteppedDiagram
  title="Orchestration saga steps"
  steps={[
    { highlight: ["Orch"], label: "Orchestrator starts the saga — it owns the workflow state machine and decides what to do next." },
    { highlight: ["Orch", "Inv"], label: "Step 1: Orchestrator sends a Reserve Stock command to the Inventory Service." },
    { highlight: ["Inv", "Orch"], label: "Step 2: Inventory Service reserves stock and responds with Stock Reserved." },
    { highlight: ["Orch", "Pay"], label: "Step 3: Orchestrator sends a Charge Card command to the Payment Service." },
    { highlight: ["Pay", "Orch"], label: "Step 4: Payment Service charges the card and responds with Card Charged." },
    { highlight: ["Orch", "Order"], label: "Step 5: Orchestrator approves the order — all three services committed independently, coordinated by the central state machine." },
  ]}
>

```mermaid
flowchart TD
    Orch[Saga Orchestrator] -->|1. Reserve Stock Command| Inv[Inventory Service]
    Inv -->|2. Stock Reserved Response| Orch
    Orch -->|3. Charge Card Command| Pay[Payment Service]
    Pay -->|4. Card Charged Response| Orch
    Orch -->|5. Approve Order Command| Order[Order Service]
```
...
In a typical Saga step, a service updates its local database and publishes an event to a message broker.
...
*   **The Mitigation**: Every command must carry a unique Saga ID and Step ID. Before processing, services must verify if the specific Step ID has already run, rejecting duplicate invocations. (See [[system-design/message-queues|Asynchronous Message Queues]] for details on distributed idempotency locks).
