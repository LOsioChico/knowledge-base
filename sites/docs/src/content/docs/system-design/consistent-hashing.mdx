...
Imagine we have 5 servers ($N = 5$) and distribute 100 session keys. If one server crashes, or if we scale up by adding a 6th server ($N = 6$), the divisor in our modulo formula changes. Because the divisor shifts, **almost every key now maps to a completely different server index.**
...
Notice how Alice's and Bob's sessions have shifted to entirely new servers. Statistically, when scaling from $N$ to $N+1$ servers, **the probability of any key staying on the same server is only $1 / (N+1)$**. 

This means:
*   When scaling from 5 to 6 servers, **$5/6$ of all keys (83.3%) are suddenly sent to the wrong servers**.
*   In a production caching layer (like Redis or Memcached), this causes a massive **cache stampede**: almost all read requests fail, forcing the application to query the primary database all at once, which frequently crashes the database.
...
Instead of dividing key hashes by `N`, it places both servers and keys onto a large, fixed circular space known as a **hash ring**. This circle is typically mapped from $0$ to $4,294,967,295$ (which is $2^{32} - 1$, the standard range of a 32-bit unsigned integer).
