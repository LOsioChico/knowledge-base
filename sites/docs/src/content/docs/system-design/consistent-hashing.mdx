...
aliases: [hash ring, virtual nodes, VNodes, consistent hash ring]
tags: [type/concept]
area: system-design
status: evergreen
source:
...
import HashRingVisualizer from '/src/components/interactive/HashRingVisualizer.astro';
...
In a distributed database or [[system-design/distributed-caching|caching]] cluster, routing requests efficiently is critical. Traditional modulo hashing maps keys directly to specific servers, but this approach breaks down under dynamic scaling. Consistent hashing, as implemented in [Amazon Dynamo](https://www.allthingsdistributed.com/files/amazon-dynamo-sosp2007.pdf) and [Apache Cassandra](https://cassandra.apache.org/doc/stable/cassandra/architecture/dynamo.html), solves this bottleneck by guaranteeing that when the node count changes, only a minimal fraction of keys require relocation.
...
[Consistent Hashing](https://dl.acm.org/doi/10.1145/258533.258660), introduced by Karger et al. in 1997, solves this scaling bottleneck by decoupling key routing from the exact number of active servers. 
...
In the interactive ring below:
*   **session-alice** sits at position 70. Traveling clockwise, the first server is **Server B** at position 140 : so alice routes to B.
*   Click **"− Remove server"** to remove Server C. Watch the routing table: only keys previously on C migrate to their new clockwise neighbor. All other keys stay put.

If Server B crashes and is removed, only the keys that mapped to Server B are affected and need to be re-routed. All other keys on the ring stay exactly where they are.

Try the interactive ring below : add or remove a server and watch which keys migrate:

<HashRingVisualizer
  title="Consistent hashing ring"
  nodes={[
    { id: "a", label: "Server A", position: 30, color: "#60a5fa" },
    { id: "b", label: "Server B", position: 140, color: "#a78bfa" },
    { id: "c", label: "Server C", position: 270, color: "#34d399" },
  ]}
  keys={[
    { id: "alice", label: "session-alice", position: 70 },
    { id: "bob", label: "session-bob", position: 160 },
    { id: "charlie", label: "session-charlie", position: 200 },
    { id: "dave", label: "session-dave", position: 310 },
    { id: "eve", label: "session-eve", position: 10 },
  ]}
  vnodes={3}
  ringSize={360}
/>
...
<CodeWalkthrough
  ...
  steps={[
    { lines: "1-10", title: "Hash function", body: "MD5 produces a hex digest. We take the first 8 hex chars (32 bits) and parse them as an unsigned integer, mapping any string key to a position between 0 and 4.3 billion on the ring.", tip: "Only 8 hex chars (32 bits) are used : the rest of the MD5 digest is discarded." },
    ...
    { lines: "29-43", title: "Adding a server (VNodes)", body: "Each physical server gets numReplicas virtual positions. We hash 'ServerName-0', 'ServerName-1', etc. to scatter VNodes across the ring. After adding, we re-sort to maintain binary search invariant.", tip: "More VNodes = better balance but more memory. 100-150 is the production sweet spot." },
    ...
    { lines: "60-84", title: "Clockwise lookup (binary search)", body: "Hash the key, then binary search sortedKeys for the first position ≥ the hash. The modulo wrap (low % length) handles the 'past the last server, go back to the first' ring behavior. This is O(log K) where K = servers × VNodes.", tip: "The modulo wrap at line 81 is what makes it a ring : overflow wraps back to position 0." },
