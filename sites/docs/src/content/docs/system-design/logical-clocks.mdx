...
aliases: [Lamport timestamps, vector clocks, happen-before, logical time]
tags: [type/concept]
area: system-design
status: evergreen
source:
  - https://lamport.azurewebsites.net/pubs/time-clocks.pdf
  - https://doi.org/10.1007/3-540-15850-1_9
related:
...
This note is part of the **[[system-design|Systems Design Track]]**. Before resolving time sync limits, make sure you understand:
...
In 1978, computer scientist Leslie Lamport proved that we do not need physical clocks to order events. Instead, we can establish order by tracking cause and effect. He defined the **happen-before relation** (denoted by `->`) in his seminal paper [Time, Clocks, and the Ordering of Events in a Distributed System](https://lamport.azurewebsites.net/pubs/time-clocks.pdf), establishing a strict basis for ordering events in distributed systems:
...
[Vector Clocks](https://doi.org/10.1007/3-540-15850-1_9) extend Lamport Timestamps to capture full causal history. Instead of a single integer, a process maintains a vector (an array) of logical clocks representing its current knowledge of all nodes in the system.
...
4.  **Receive Message**: When process `P_j` receives a message carrying vector `W`, it merges the incoming vector:
...
| Guarantees & Cost | [Lamport Timestamps](#lamport-timestamps) | [Vector Clocks](#vector-clocks) |
| :--- | :--- | :--- |
| **Clock State Size** | O(1) (Single integer counter; see [algorithm](#lamport-algorithm)) | O(N) (Integer array for N processes; see [algorithm](#vector-algorithm)) |
| **Causal Ordering** | Weak: If `a -> b`, then `C(a) < C(b)`. Converse is false (see [Lamport limitation](#the-limitation-of-lamport-timestamps)). | Strong: `a -> b` if and only if `V(a) < V(b)`. |
| **Conflict Detection** | Impossible. Concurrent events receive overlapping timestamps. | Perfect. Detects concurrent updates when vectors are incomparable (see [Detecting Conflicts](#detecting-conflicts-and-concurrency)). |
| **Message Overhead** | Negligible (Adds a single integer to each message; see [Send step](#lamport-algorithm)) | Scalability Bottleneck (Adds N integers to each message; see [overhead discussion](#gotchas-and-production-considerations)) |
