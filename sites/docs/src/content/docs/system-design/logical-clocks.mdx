...
[Vector Clocks](https://www.vs.inf.ethz.ch/publ/papers/VirtTimeGlobStates.pdf) extend Lamport Timestamps to capture full causal history. Instead of a single integer, a process maintains a vector (an array) of logical clocks representing its current knowledge of all nodes in the system.
