...
*   **Use Case**: Ephemeral metrics, telemetry, or sensor logs where speed is critical and minor data loss is acceptable.
...
*   **Malformed payloads**: The message payload has syntax errors or violates the expected JSON schema.
*   **Business validation failures**: The message contains references to missing resources (like a deleted user).
