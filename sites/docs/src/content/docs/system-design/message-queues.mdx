...
This note is part of the **[Systems Design Track](/knowledge-base/system-design/)**. Before queuing messages, make sure you understand:
...
| :--- | :--- | :--- |
| **Storage Engine** | Volatile, transient queues (deleted on ACK) | Durable, append-only sequential log on disk |
| **Consumer State** | Tracked by the broker (ACKs, locks) | Tracked by the consumer (log read offset) |
| **Message Replay** | No (destructive consumption) | Yes (rewind offset to reread history) |
| **Routing Power** | High (exchange routing, wildcards) | Low (routing is determined by Partition keys) |
| **Parallelism Scale** | Limited by single-queue locking overhead | Massive (horizontal partitions scale out) |
| **Typical Tech** | RabbitMQ, SQS, ActiveMQ | Apache Kafka, Apache Pulsar, Kinesis |
...
export class IdempotentConsumer {
  private readonly redisClient: any; // Redis client wrapper
...
*   **Malformed payloads**: The message payload has syntax errors or violates the JSON [[effect-ts/schema|schema]].
