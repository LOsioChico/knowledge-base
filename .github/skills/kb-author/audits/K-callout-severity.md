...
- **Neat, didn't know that** → info, or plain prose if it does not need visual weight.

Real wild example caught in the wild: this audit fired on April 28, 2026 with 41 `[!warning]`
callouts vs 24 `[!info]`. Warnings outnumbered every other type — almost always a sign of
severity inflation. After the sweep: 27 warnings, with the rest demoted to info.
...
- "Alternative-comparison" ("don't use X as a substitute for Y") → **info** (advisory, not
  failure).
- "Convention guidance" ("pick one prefix and stick with it") → **info** or plain prose.
