...
**Fails** if the MDX page could have been produced by copying raw markdown into `sites/docs/` with
minimal find-replace. **Passes** if the page is structured for someone who has **never** opened the
source notes.

## Paste test (30 seconds)

Read the MDX page cold. Ask:
...
3. Are audit-only concerns (long audit chains, `related:` dumps, discoverability asides) **absent**?
4. Is anything **shorter** than the source because redundancy was cut, not because facts were dropped?
...
- Every gotcha callout — keep highest-signal footguns as `<Aside>`
...
- Pasting raw markdown with only callout/HTML rewrites — that is not re-authoring.

## Source accuracy

Facts must still match verified sources. If MDX **simplifies**, the simplification must remain
**true**. The published page must cite primary sources inline or in prose links.
