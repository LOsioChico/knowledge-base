...
How Starlight MDX and GitHub Pages fit together. **Source of truth for commands:** root `package.json` and `.github/workflows/deploy.yml`.
...
| --- | --- | --- |
| Wikilinks | `bun run lint:wikilinks` | `sites/docs/src/content/docs/` — symmetry, first-mention, discoverability, tagline, agents-mirror |
| MDX page count | `bun run lint:publish-parity` | MDX page count health check |
| Pass 0 + format | `cd scripts/audit-notes && bun run lint:content && bun run lint:format` | Em-dash, double-hyphen, Prettier on MDX |
...
**Published coverage:** MDX under `sites/docs/` is canonical. `bun run lint:publish-parity` runs as an MDX page count health check.
...
| MDX post-edit | `bun run vault:check --base HEAD~1` — parity, diff-scoped MDX Pass 0, `lint:docs` when `sites/docs/` changed, optional `mdx-triage` when `CURSOR_API_KEY` is set |
...
| --- | --- | --- |
| `lint:wikilinks` | `sites/docs/src/content/docs/**/*.mdx` | — |
| `lint:mdx-wikilinks` | `sites/docs/src/content/docs/**/*.mdx` | — |
...
| `lint:docs` | Starlight typecheck + all MDX linters | — |
| `vault:check` | Diff-scoped MDX + optional `lint:docs` | Full wikilink pass (use `lint:wikilinks` separately) |
...
- **Backlinks** — Starlight pages get a build-time `## Backlinks` section.
...
publish parity, diff-scoped MDX Pass 0, `lint:docs` when `sites/docs/` changed,
optional `mdx-triage` when `CURSOR_API_KEY` is set, and skips Prettier. Run `bun run lint:ci` for the full linter-only CI chain.
...
- [`AGENTS.md`](../AGENTS.md) — authoring contract
