...
export function formatHumanReport(report) {
  ...
  if (report.changedFiles.length === 0) {
    ...
    lines.push(
      ...formatAuditTotalsBlock("mdx-audit (triage)", report.mdxAudit, report.mdxAuditSkipped),
    );
  ...
  lines.push(
    links.ok
      ? "✓ wikilinks (scoped): clean"
      : `✗ wikilinks (scoped): ${links.violations.length} violation(s)`,
  );
  if (!links.ok) {
