...
- `tech/typescript`, `tech/rxjs`, `tech/multer`, `tech/http`, `tech/class-validator`, `tech/class-transformer`, `tech/asynclocalstorage`, `tech/nest-cli`, `tech/typeorm`, `tech/postgres`, `tech/kafka` (reserved), `tech/prisma` (reserved), `tech/jwt` (reserved)
...
- **Inline-source-citations rule** (same linter, BLOCKING): every inline link in note bodies pointing at a primary-source URL (currently `https://github.com/<owner>/<repo>/blob/...` and `https://docs.nestjs.com/...`) MUST have its fragment-stripped form present in the note's frontmatter `source:` list. This is the deterministic safety net for the LLM audit's "Plausible but unsourced" findings: when you cite `parse-date.pipe.ts#L10-L31` inline next to a claim, the bare file URL `.../parse-date.pipe.ts` must also appear in `source:`. Fragment (`#L<m>-L<n>`, `#section-anchor`) and trailing slash are stripped before comparison, so the inline link can carry precision while `source:` stays file-level (keeps `check-source-urls.sh` cheap). Forbidden: dropping a `([source](https://github.com/.../foo.ts#L20-L40))` next to a claim and skipping the matching `source:` entry — the audit will eventually flag the bare claim as unsourced (because Pass 1d only greps `source:` URLs, not inline links), and the next reader has no way to discover the rest of that file's relevant context. Required: when adding any inline citation, also add the fragment-stripped URL to `source:` in the same edit. The linter fails CI on missing entries; add new domain prefixes to `PRIMARY_SOURCE_RE` in `quartz/scripts/lint-wikilinks-core.mjs` when the vault grows beyond NestJS sources.
